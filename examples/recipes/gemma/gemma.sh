...
rank[g293.cluster.local]=6

rank[g294.cluster.local]=7
...
rank[g298.cluster.local]=4

#rank[g299.cluster.local]=0

rank[g300.cluster.local]=5
...
rank[g314.cluster.local]=0

rank[g315.cluster.local]=1

rank[g316.cluster.local]=2

rank[g317.cluster.local]=3
...
MASTER_PORT=6969 # An open port for communication
...
CHECKPOINT_PATH="/checkpoints/sbd4" # Example path
DATA_PATH="/datasets/processed_qwen3/tokenizer_dataset/zyda2/train_text_document" # Path PREFX for your .bin/.idx files
LOG_DIR="/logs/my_qwen_diffusion_run" # Example log directory
RUN_NAME="qwen_600m_diffusion_test"
...
MBRIDGE_ARGS=(
    ...
    train.train_iters=55000 # Short test run
    train.eval_interval=1000000000000
    train.eval_iters=0
    model.seq_length=2048
    dataset.sequence_length=2048
    # Checkpointing
    checkpoint.pretrained_checkpoint="/checkpoints/sbd4" # Load from this path initially
    checkpoint.load=$CHECKPOINT_PATH      
    checkpoint.save=$CHECKPOINT_PATH              # Save subsequent checkpoints HERE
    checkpoint.save_interval=2500
    # checkpoint.load=$CHECKPOINT_PATH
    #+checkpoint.load_lr_scheduler_states=False
    ...
    scheduler.lr_warmup_iters=250 
    scheduler.lr_decay_style=cosine
    scheduler.lr_decay_iters=55000 
    ...
    train.global_batch_size=1280 # GBS = MBS * DP = 1 * 8 (since TP=1, PP=1)

    # Optimizer
    optimizer.lr=2.5e-5
    optimizer.min_lr=7.5e-7
    optimizer.clip_grad=10.0
    optimizer.weight_decay=0.1
    #+optimizer.lr_decay='constant'
